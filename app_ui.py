#!/usr/bin/env python3
"""Streamlit UI runner for interactive execution (Google Antigravity compatible)."""
import shutil
import sys
import tempfile
...
with col1:
    ...
    input_method = st.radio("Provide files via:", ["Select Existing Folder on Disk", "Upload Loose Files directly"])

    source_dir = None
    if input_method == "Select Existing Folder on Disk":
        default_folder = str((Path(__file__).resolve().parent / "data" / "inputs" / "raw_archive").resolve())
    ...
    default_out = str((Path(__file__).resolve().parent / "data" / "outputs").resolve())
    ...
    move_mode = st.checkbox("Move files instead of copying (destructively saves disk space)", value=False)
